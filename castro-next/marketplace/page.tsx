...
export default function Marketplace() {
  ...
  const [uploadForm, setUploadForm] = useState({
    name: "", description: "", price: "", category: "NLP",
  });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      form.append("name", uploadForm.name.replace(/\s+/g, "-"));
  ...
  const handlePurchase = async (dataset: Dataset) => {
    try {
      const res = await fetch(`${API_URL}/purchase/${dataset.blobName}`, {
        ...
        body: JSON.stringify({ buyerAddress: "0x1234...demo" }),
      });
      const data = await res.json();
      if (data.success) {
        setShowBuy(null);
  ...
  const handleDownload = async (blobName: string) => {
    try {
      const res = await fetch(`${API_URL}/download/${blobName}`);
  ...
  return (
    <div style={{ background: bg, color: tx, minHeight: "100vh", fontFamily: "var(--font-geist-sans, sans-serif)" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 24px", borderBottom: `1px solid ${bdr}`, background: bg, flexWrap: "wrap", gap: 10, position: "sticky", top: 0, zIndex: 50 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <div style={{ width: 32, height: 32, background: green, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, color: "#fff", fontFamily: "monospace", fontSize: 15 }}>D</div>
          <span style={{ fontSize: 15, fontWeight: 700, color: tx, letterSpacing: "-0.3px" }}>DataVault</span>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <div style={{ display: "flex", background: bg2, border: `1px solid ${bdr2}`, borderRadius: 10, padding: 3, gap: 2 }}>
            {["dark", "light"].map(t => (
              <button key={t} onClick={() => setTheme(t)} style={{ fontSize: 11, padding: "5px 11px", borderRadius: 7, border: "none", cursor: "pointer", background: theme === t ? green : "transparent", color: theme === t ? "#fff" : tx2, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 4 }}>
                {t === "dark" ? "🌙" : "☀️"} {t.charAt(0).toUpperCase() + t.slice(1)}
              </button>
            ))}
          </div>
          <button onClick={() => setShowUpload(true)} style={{ fontSize: 12, padding: "7px 16px", borderRadius: 9, background: green, border: "none", color: "#fff", cursor: "pointer", fontWeight: 600, fontFamily: "inherit" }}>
            + Upload dataset
          </button>
        </div>
      </nav>
      ...
      {/* HERO */}
      <div style={{ padding: "60px 24px 44px", textAlign: "center", position: "relative" }}>
        ...
        <p style={{ fontSize: 14, color: tx2, maxWidth: 460, margin: "0 auto 28px", lineHeight: 1.75 }}>
          Every dataset stored on Shelby Protocol — hot decentralized storage on Aptos. Buy and sell AI training data with ShelbyUSD.
        </p>
      ...
      {/* STATS */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 0, border: `1px solid ${bdr}`, borderRadius: 12, overflow: "hidden", margin: "0 24px 28px" }}>
      ...
      <div style={{ display: "flex", borderBottom: `1px solid ${bdr}`, padding: "0 24px", background: bg, overflowX: "auto" }}>
        {[["shelby", "📦 DataVault listings"], ["discover", "🌐 Discover external"], ["about", "ℹ️ How it works"]].map(([id, label]) => (
          <button key={id} onClick={() => setTab(id)} style={{ fontSize: 12, padding: "12px 16px", color: tab === id ? green : tx2, cursor: "pointer", border: "none", background: "transparent", borderBottom: tab === id ? `2px solid ${green}` : "2px solid transparent", fontFamily: "inherit", fontWeight: 600, whiteSpace: "nowrap" }}>
            {label}
          </button>
        ))}
      </div>
        ...
        <div style={{ padding: "28px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
            <div>
              ...
              <div style={{ fontSize: 20, fontWeight: 700, color: tx, letterSpacing: "-0.4px" }}>Featured datasets</div>
            </div>
          ...
          <div style={{ display: "flex", gap: 7, flexWrap: "wrap", marginBottom: 18 }}>
            {categories.map(c => (
              <button key={c} onClick={() => setCategory(c)} style={{ fontSize: 11, padding: "5px 13px", borderRadius: 8, cursor: "pointer", border: `1px solid ${category === c ? green : bdr2}`, background: category === c ? green : "transparent", color: category === c ? "#fff" : tx2, fontFamily: "inherit", fontWeight: 600 }}>
                {c === "all" ? "All" : c}
              </button>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 12 }}>
              {datasets.map((d, i) => (
                <div key={i} style={{ background: card, border: `1px solid ${bdr}`, borderRadius: 12, padding: "1.1rem", display: "flex", flexDirection: "column", gap: 9, cursor: "pointer", transition: "all 0.2s" }}
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 6, background: green2, color: green, fontWeight: 700, display: "flex", alignItems: "center", gap: 3 }}>☁️ Shelby</span>
                  </div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: tx, letterSpacing: "-0.2px" }}>{d.blobName}</div>
      ...
      {tab === "discover" && (
        <div style={{ padding: "28px 24px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(210px,1fr))", gap: 12 }}>
            {[
              { name: "Common crawl news 2024", tag: "NLP", src: "HuggingFace", srcColor: "#d97706", desc: "800GB news articles for pretraining LLMs.", size: "800 GB", free: true },
              { name: "Open images v7", tag: "Vision", src: "Kaggle", srcColor: "#3b82f6", desc: "9M images with 36M bounding boxes.", size: "565 GB", free: true },
              { name: "SQuAD 2.0", tag: "NLP", src: "PwC", srcColor: "#ef4444", desc: "100K Q&A pairs for reading comprehension.", size: "35 MB", free: true },
              { name: "Common voice 17", tag: "Audio", src: "HuggingFace", srcColor: "#d97706", desc: "2,500+ hours across 120 languages.", size: "70 GB", free: true },
              { name: "House prices advanced", tag: "Tabular", src: "Kaggle", srcColor: "#3b82f6", desc: "79 features for residential price prediction.", size: "1 MB", free: true },
              { name: "MIMIC-III clinical notes", tag: "Medical", src: "GitHub", srcColor: tx3, desc: "40K ICU patient records for clinical NLP.", size: "6.7 GB", free: false },
            ].map((d, i) => (
              <div key={i} style={{ background: card, border: `1px solid ${bdr}`, borderRadius: 12, padding: "1.1rem", display: "flex", flexDirection: "column", gap: 9 }}
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 6, background: `${d.srcColor}18`, color: d.srcColor, fontWeight: 700 }}>{d.src}</span>
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: tx }}>{d.name}</div>
                <div style={{ fontSize: 11, color: tx2, lineHeight: 1.55 }}>{d.desc}</div>
                <div style={{ display: "flex", alignItems: "center", gap: 5, background: `${d.srcColor}10`, border: `1px solid ${d.srcColor}30`, borderRadius: 7, padding: "5px 8px" }}>
                  <span style={{ fontSize: 10, color: d.srcColor, fontWeight: 600 }}>🔗 {d.src} · {d.size} · {d.free ? "Free" : "Credentialed"}</span>
                </div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", borderTop: `1px solid ${bdr}`, paddingTop: 9, marginTop: "auto" }}>
      ...
      {tab === "about" && (
        <div style={{ padding: "28px 24px" }}>
          ...
          <div style={{ fontSize: 20, fontWeight: 700, color: tx, marginBottom: 6 }}>Powered by Shelby Protocol</div>
          <p style={{ fontSize: 13, color: tx2, marginBottom: 20, lineHeight: 1.65, maxWidth: 460 }}>Shelby is decentralized hot storage on Aptos — fast like S3, trustless like a blockchain.</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(160px,1fr))", gap: 12, marginBottom: 28 }}>
            {[["🔓", "Censorship resistant", "No central server can delete your dataset. Ever."], ["⚡", "Hot storage", "Shelby serves data fast — not IPFS-slow."], ["💰", "ShelbyUSD payments", "On-chain payments. Sellers earn instantly."], ["🛡️", "On-chain ownership", "Every dataset registered on Aptos."]].map(([ico, t, d]) => (
              <div key={t} style={{ background: card, border: `1px solid ${bdr}`, borderRadius: 12, padding: "1.1rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(130px,1fr))", gap: 10 }}>
            {[["01", "Connect Petra", "Your wallet is your identity on DataVault."], ["02", "Get ShelbyUSD", "Claim free tokens from the testnet faucet."], ["03", "Buy a dataset", "Pay SUSD, download unlocks instantly."], ["04", "Upload & earn", "Upload to Shelby, set price, earn on every sale."]].map(([n, t, d]) => (
              <div key={n} style={{ background: dark ? "rgba(255,255,255,0.02)" : "#f8fafc", border: `1px solid ${bdr}`, borderRadius: 12, padding: 14 }}>
      ...
      {/* CTA */}
      <div style={{ background: `linear-gradient(135deg,${green2},rgba(16,185,129,0.04))`, border: `1px solid ${green3}`, borderRadius: 14, padding: 28, textAlign: "center", margin: "0 24px 28px" }}>
        <h2 style={{ fontSize: 20, fontWeight: 800, marginBottom: 8, color: tx, letterSpacing: "-0.4px" }}>Start selling your datasets on Shelby</h2>
      ...
      <footer style={{ padding: "16px 24px", borderTop: `1px solid ${bdr}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8, background: bg }}>
        ...
        <div style={{ display: "flex", gap: 16 }}>
          {["Twitter", "Docs", "GitHub", "Faucet"].map(l => (
            <a key={l} style={{ fontSize: 11, color: tx3, cursor: "pointer", textDecoration: "none", fontWeight: 500 }}>{l}</a>
          ))}
        </div>
      </footer>
      ...
      {showUpload && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }}>
          <div style={{ background: card, border: `1px solid ${bdr2}`, borderRadius: 14, padding: "1.5rem", width: 320, maxWidth: "100%", position: "relative" }}>
            <button onClick={() => setShowUpload(false)} style={{ position: "absolute", top: 13, right: 13, background: bg3, border: "none", color: tx2, cursor: "pointer", fontSize: 14, width: 26, height: 26, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "inherit" }}>✕</button>
            <div style={{ fontSize: 14, fontWeight: 700, color: tx, marginBottom: 14, display: "flex", alignItems: "center", gap: 7 }}>📤 Upload dataset to Shelby</div>
            <div onClick={() => document.getElementById("ds-file")?.click()} style={{ border: `2px dashed ${bdr2}`, borderRadius: 10, padding: "1.3rem", textAlign: "center", cursor: "pointer", marginBottom: 4 }}>
              ...
              <p style={{ fontSize: 12, color: tx2, margin: "0 0 3px", fontWeight: 600 }}>{file ? file.name : "Drag & drop or click to browse"}</p>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: 20 }}>
          <div style={{ background: card, border: `1px solid ${bdr2}`, borderRadius: 14, padding: "1.5rem", width: 310, maxWidth: "100%", position: "relative" }}>
            ...
            <div style={{ border: `1px solid ${bdr}`, borderRadius: 10, overflow: "hidden", marginBottom: 14 }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "10px 12px", background: green2, color: tx }}>
                <span style={{ fontWeight: 700 }}>Your balance</span><span style={{ color: green, fontWeight: 700 }}>240 SUSD</span>
              </div>
            </div>
            <div style={{ background: green2, border: `1px solid ${green3}`, borderRadius: 8, padding: "9px 11px", display: "flex", alignItems: "center", gap: 7, marginBottom: 12 }}>
              <span>🛡️</span><span style={{ fontSize: 11, color: tx2, lineHeight: 1.5 }}>Download unlocks instantly after payment is confirmed on Aptos.</span>
            </div>
